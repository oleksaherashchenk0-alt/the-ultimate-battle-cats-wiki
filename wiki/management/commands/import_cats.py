        ...
        with f:
            ...
            with transaction.atomic():
                for i, row in enumerate(reader, start=2):  # start=2, бо рядок 1 - заголовки
                    ...
                    status = (row.get('status') or 'normal').strip()
                    if status not in VALID_STATUSES:
                        skipped_rows.append((i, f"невідомий status '{status}' у кота '{title}'"))
                        continue

                    defaults = {'status': status, 'author': author}
                    for csv_col, (field_name, caster) in FIELD_MAP.items():
                        if csv_col == 'status':
                            continue
                        if csv_col in row:
                            try:
                                defaults[field_name] = caster(row.get(csv_col))
