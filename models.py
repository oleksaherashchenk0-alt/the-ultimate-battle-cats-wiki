...
class CatForm(models.Model):
    """
    Форма еволюції кота (Base -> Evolved -> True -> Ultra). Кожен кіт може
    мати від 1 до 4 таких форм - на сторінці кота вони показуються як
    перемикач (вкладки), і при натисканні міняються ім'я/іконка/статті,
    які відрізняються між формами. Дальність/швидкість/ціна/трейти/
    здібності зазвичай однакові для всіх форм - тому лишаються на самому
    BattleCat і не дублюються тут.
    """
    STAGE_CHOICES = [
        ('normal', 'Base Form'),
    ...
    cat = models.ForeignKey('BattleCat', on_delete=models.CASCADE, related_name='forms', verbose_name="Кіт")
    stage = models.CharField("Стадія еволюції", max_length=10, choices=STAGE_CHOICES)
    name = models.CharField("Назва форми", max_length=150)
    icon = models.ImageField("Іконка форми", upload_to='cat_forms/', blank=True, null=True)

    hp_stat = models.IntegerField("Здоров'я (HP)", default=0)
    attack_power = models.IntegerField("Урон (Attack Power)", default=0)
    dps_stat = models.FloatField("DPS", default=0.0)

    name_de = models.CharField("Назва (DE)", max_length=150, blank=True)
    name_es = models.CharField("Назва (ES)", max_length=150, blank=True)
    name_ja = models.CharField("Назва (JA)", max_length=150, blank=True)

    order = models.PositiveSmallIntegerField("Порядок показу", default=0)
    ...
    def __str__(self):
        return f"{self.cat.title} — {self.get_stage_display()}"


class BattleCat(models.Model):
    title = models.CharField(max_length=150)
    slug = models.SlugField(
        "URL-адреса (латиницею, генерується автоматично з назви)",
        max_length=170, unique=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        # Генеруємо slug автоматично з назви кота (одноразово, коли він
        # ще порожній) - так URL виглядає як /cat/dark-cat/ замість
        # /cat/4/, і краще індексується пошуковиками (SEO).
        if not self.slug:
            from django.utils.text import slugify
            base_slug = slugify(self.title) or 'cat'
            slug = base_slug
            n = 1
            while BattleCat.objects.filter(slug=slug).exclude(pk=self.pk).exists():
                n += 1
                slug = f"{base_slug}-{n}"
            self.slug = slug
