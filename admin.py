...
from .models import BattleCat, Ability, CatForm


class CatFormInline(admin.TabularInline):
    # Форми еволюції редагуються прямо тут, всередині картки кота -
    # додай рядок на кожну стадію, яку хочеш показати (Base/Evolved/
    # True/Ultra), не обов'язково всі 4 одразу.
    model = CatForm
    extra = 1
    fields = ('stage', 'name', 'icon', 'hp_stat', 'attack_power', 'dps_stat', 'order')
...
class BattleCatAdmin(admin.ModelAdmin):
    ...
    # При наборі назви кота в адмінці slug підставляється сам (можна
    # й вручну підправити, якщо треба інший вигляд URL)
